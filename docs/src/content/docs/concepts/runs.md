...
| audit | `squirrel audit`, `squirrel verify`, `squirrel runs fail` (a `manual-fail` audit row), and the [agent](/squirrel/guides/agent/) [reloading its config](/squirrel/guides/agent/#config-edits-apply-themselves) or latching the part of an edit that still needs a restart |
