...
// reset cleared (or, on a dry run, would clear). The five categories are the
// full derived record squirrel keeps about a destination: the per-content,
// per-pack and per-mirror-path upload ledgers, the live durability vector,
// and the push-freshness maxima. They span every volume — a destination's recorded
...
// Total is the sum of all five categories, used as the reset run's
// file_count so the audit trail carries the magnitude at a glance.
func (c DestinationResetCounts) Total() int64 {
	return c.RemoteObjects + c.RemotePacks + c.RemotePaths + c.VectorComponents + c.FreshnessRows
}
...
func countDestinationRecordedState(ctx context.Context, q rowQuerier, destination string) (DestinationResetCounts, error) {
	...
	counts := []struct {
		...
		{&c.RemotePaths, `SELECT COUNT(*) FROM remote_paths WHERE destination = ?`},
...
// remote state — its per-content, per-pack and per-mirror-path upload
// ledgers, its live durability vector, and its push-freshness maxima — so the next sync treats
...
func (s *Store) ResetDestination(ctx context.Context, destination string) (int64, DestinationResetCounts, error) {
	...
	note := fmt.Sprintf("destination=%q objects=%d packs=%d paths=%d vector=%d freshness=%d",
		destination, counts.RemoteObjects, counts.RemotePacks, counts.RemotePaths, counts.VectorComponents, counts.FreshnessRows)
...
// deleteDestinationRecordedStateTx clears the five derived-state tables for
...
func deleteDestinationRecordedStateTx(ctx context.Context, tx *sql.Tx, destination string) error {
	stmts := []string{
		...
		`DELETE FROM remote_paths WHERE destination = ?`,
